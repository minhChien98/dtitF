...
class AdminPage extends Component {
  ...
  componentWillReceiveProps(props) {
    ...
    if (questionList && questionList.success) {
      ...
      this.props.onGetQuestionInList(
        questionList.questionList[this.state.currentList]._id
      );
  ...
  render() {
    ...
    return (
      <div className="col-md-6 col-sm-11 container">
        ...
        <button
          disabled={this.state.ans}
          className="btn btn-primary"
          style={{ width: "100%" }}
          onClick={this.raiseAns}
        >
          Trả kết quả
        </button>
        <button
        ...
        <button
          disabled={this.state.canStartHelp}
          className="btn btn-primary"
          style={{ width: "100%" }}
  ...
  startGame = () => {
    window.socketIO.emit("admin", {
      message: this.state.questions[0].questId._id,
      command: 1002
    });
  ...
  startHelp = () => {
    ...
    window.socketIO.emit("admin", {
      message: this.state.questions[indexQues].questId._id,
      command: 3001
    });
  ...
  nextQuestion = () => {
    ...
    window.socketIO.emit("admin", {
      message: this.state.questions[indexQues + 1].questId._id,
      command: 1000
    });
  };
...
const mapDispatchToProps = dispatch => {
  return {
    ...
    onGetQuestionInList: listId =>
      dispatch({ type: types.GET_QUESTION_IN_LIST, listId }),
